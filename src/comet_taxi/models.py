...
class RunningMeanStd(nn.Module):
    ...
    def update(self, values: torch.Tensor | np.ndarray) -> None:
        ...
        tensor = tensor.detach().to(device=self.mean.device, dtype=torch.float32).reshape(-1, self.mean.shape[0])
        if tensor.numel() == 0:
            return
        batch_mean = tensor.mean(dim=0)
        batch_var = tensor.var(dim=0, unbiased=False)
        batch_count = torch.tensor(float(tensor.shape[0]), device=self.mean.device)
